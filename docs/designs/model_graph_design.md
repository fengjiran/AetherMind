...
`ModelWeightBinding` 不拥有权重数据，也不持有指向权重数据的裸指针。它仅保存 `(role, layer_index)` 二元组作为查找键，lowering 阶段通过此键从 `ModelInstance::GetResolvedWeights()` 解析实际的 `RawWeightView`。这避免了 `ResolvedModelWeights` 内部 vector reallocation 导致的悬垂指针风险。
...
4. 根据 `ModelWeightBinding` 的 `(role, layer_index)` 从 `ModelInstance::GetResolvedWeights()` 解析实际权重引用；
...
2. **权重绑定到投影节点**：`kLinear` 节点通过 `ModelWeightBinding` 绑定对应的权重角色（如 `kAttentionQ`），而非将权重作为额外输入张量。
...
```
N_total = 1 + num_hidden_layers × 13 + 3
```

对于 Llama-2-7B（32 层）：`1 + 32 × 13 + 3 = 420` 个节点。
...
`ModelGraph` 只描述某个节点需要什么 weight role，不直接持有权重数据或 packed weight storage。`ModelWeightBinding` 仅保存 `(role, layer_index)` 查找键，实际权重解析在 lowering 阶段完成：
...
- `ModelWeightBinding` 与 `ResolvedModelWeights` 的内存布局完全解耦，不存在悬垂风险；
