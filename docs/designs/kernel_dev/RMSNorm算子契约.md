...
版本：v0.2<br>
状态：当前实现契约<br>
适用范围：AetherMind CPU FP32 RMSNorm<br>
更新时间：2026-08-31
...
$$
\operatorname{mean\_square}_r = \frac{1}{H}\sum_{h=0}^{H-1} X_{r,h}^2
$$

$$
Y_{r,h} = X_{r,h} \cdot \frac{1}{\sqrt{\operatorname{mean\_square}_r + \varepsilon}} \cdot \gamma_h
$$
...
CpuBackend::PrepareKernel
    ...
    -> BuildRmsNormKernelParams
    -> RmsNormKernelEntryFp32Scalar / RmsNormKernelEntryFp32Avx2Fma
    -> ValidateAndBuildRmsNormFp32Args
    -> RunRmsNormFp32Scalar / RunRmsNormFp32Avx2Fma
```

- `RmsNormKernelParams` 保存调用方借用的 `TensorView`；其生命周期必须覆盖同步 kernel entry 调用。
- `ValidateAndBuildRmsNormFp32Args` 是 FP32 entry 的唯一运行时验证路径，负责 epsilon、dtype、rank、shape、row count、layout、pointer 和 stride 检查。
- `RmsNormFp32KernelArgs` 是只供已验证 compute primitive 使用的内部类型；compute 不重复验证，也不拥有内存。
