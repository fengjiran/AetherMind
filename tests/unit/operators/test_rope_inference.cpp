...
TEST(RoPEInference, AcceptsValidParams) {
...
TEST(RoPEInference, RejectsInvalidParams) {
    auto q = MakeSpec(DataType::Float32(), {1, 128, 32 * 64});
    auto k = MakeSpec(DataType::Float32(), {1, 128, 8 * 64});
    auto pos = MakeSpec(DataType::Int(64), std::vector<int64_t>{128});
    std::vector<TensorSpec> inputs = {q, k, pos};
...
TEST(RoPEInference, RejectsInvalidTheta) {
    auto q = MakeSpec(DataType::Float32(), {1, 128, 32 * 64});
    auto k = MakeSpec(DataType::Float32(), {1, 128, 8 * 64});
    auto pos = MakeSpec(DataType::Int(64), std::vector<int64_t>{128});
    std::vector<TensorSpec> inputs = {q, k, pos};
...
TEST(RoPEInference, PreservesFloat32OutputDType) {
    auto q = MakeSpec(DataType::Float32(), {1, 128, 32 * 64});
    auto k = MakeSpec(DataType::Float32(), {1, 128, 8 * 64});
    auto pos = MakeSpec(DataType::Int(64), std::vector<int64_t>{128});
    RoPEParams p{64, 32, 8, 2048};
...
TEST(RoPEInference, AcceptsGQACompatible) {
...
TEST(RoPEInference, AcceptsQLastDimMismatch) {
...
TEST(RoPEInference, AcceptsKLastDimMismatch) {
    auto q = MakeSpec(DataType::Float32(), {1, 128, 32 * 64});
...
TEST(RoPEInference, RejectsRankMismatch) {
    auto q = MakeSpec(DataType::Float32(), {1, 128, 32 * 64});
...
TEST(RoPEInference, RejectsBatchDimMismatch) {
