...
TensorSpec Tensor(DataType dtype, std::vector<ShapeSymbol> dims) {
...
TensorSpec WeightTensor(const RawWeightView& weight) {
    return Tensor(weight.dtype, SymbolicDims(weight.shape));
}

TensorSpec ActivationTensor(DataType dtype, ShapeSymbol seq_len, int64_t feature_dim) {
    return Tensor(dtype, {seq_len, ShapeSymbol::CreateFromValue(feature_dim)});
}
...
StatusOr<ModelGraph> ModelGraphBuilder::BuildLlamaDense(
        const HfModelConfig& config,
        const ResolvedModelWeights& weights) {
    AM_RETURN_IF_ERROR(ValidateInputs(config, weights));

    const DataType activation_dtype = config.weight_dtype_hint.bits() > 0 ? config.weight_dtype_hint : DataType::Float32();
    ...
    const TensorSpec token_ids = Tensor(DataType::Int(64), {seq_len});
    const TensorSpec hidden = ActivationTensor(activation_dtype, seq_len, hidden_size);
    const TensorSpec kv_hidden = ActivationTensor(activation_dtype, seq_len, kv_hidden_size);
    const TensorSpec intermediate = ActivationTensor(activation_dtype, seq_len, config.intermediate_size);
    const TensorSpec scores = Tensor(activation_dtype, {seq_len, seq_len});
    const TensorSpec logits = ActivationTensor(activation_dtype, seq_len, config.vocab_size);
