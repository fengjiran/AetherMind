...
TEST(StringFindTest, ExceptionHandling) {
    ...
    // 这里测试pos参数的边界情况
    EXPECT_NO_THROW(str.find("t", 5, 1));
}
...
// 边界情况测试
TEST(StringFindTest, BoundaryCases2) {
    // 空字符串
    String empty;
...
// 特殊字符测试
TEST(StringFindTest, SpecialCharacters2) {
    // 包含空格、制表符、换行符
    String str("space\ttab\nnewline");
...
// 异常处理测试
TEST(StringFindTest, ExceptionHandling2) {
    String str("test");

    // 测试pos参数越界
    EXPECT_NO_THROW(str.find(String("t"), 5));
    EXPECT_NO_THROW(str.find("t", 5));
    EXPECT_NO_THROW(str.find('t', 5));
}

// COW机制测试
TEST(StringFindTest, CopyOnWrite2) {
    String original("test string for COW");
    String copy = original;// 应该共享数据
...
// 组合操作测试
TEST(StringFindTest, CombinedOperations2) {
    String str("abcdefghijklmnopqrstuvwxyz");
