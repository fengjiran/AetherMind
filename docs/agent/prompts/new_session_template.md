...
> 默认启动契约、最小加载路径、确认闸门与 Resume Gate，统一以 `AGENTS.md` 第12节为准。
...
- 在通过 Resume Gate 前，只允许根目录 `AGENTS.md` 第12节定义的只读引导操作
- 加载完成后，必须等待用户明确说“继续”/“执行”/“是”，才可执行非恢复/业务工具操作
- 在此之前，禁止与恢复无关的代码扫描、构建、测试、编辑、写入及任何外部副作用操作
