# 产品需求文档 (PRD): AetherMind 大模型推理引擎

| 文档版本 | V1.1 |
|----------|----------------|
| **项目名称** | **AetherMind** |
| **核心目标** | 交付生产级 CPU 嵌入式推理引擎 (Phase 1)，并为服务化/分布式演进预留架构空间 |
...
本文档采用**双层结构**：
- **Phase 1（本文档主体）**：可执行的详细产品合同，聚焦 CPU 嵌入式运行时
...
|--------|--------------|--------------|
| **边缘部署** | CPU INT8/INT4 量化，4GB 内存运行 7B 模型 | Phase 2: GPU 卸载 |
| **确定性输出** | 贪婪采样 + 数值稳定的 CPU 参考内核 | Phase 2+: 支持 temperature/top-p |
| **低延迟启动** | 静态预分配，预热后零分配 | Phase 2+: PagedAttention 动态管理 |
| **API 稳定** | 版本化 C ABI (v1.0)，向前兼容优先 | Phase 2+: HTTP API |

### 1.3 目标用户

- **嵌入式设备开发者**：需要在边缘设备运行 LLM 的应用
...
│  ┌────────────┐ ┌────────────┐ ┌────────────┐                  │
│  │   Model    │ │   Sync     │ │   Static   │                  │
│  │  Manager   │ │  Executor  │ │  KV Cache  │                  │
│  │(Load/Store)│ │(Prefill/   │ │(Prealloc)  │                  │
│  │            │ │ Decode)    │ │            │                  │
│  └────────────┘ └────────────┘ └────────────┘                  │
│  ┌────────────┐ ┌────────────┐                                 │
│  │  Loader    │ │ Dispatch   │                                 │
...
- 无动态内存：Decode 循环零分配（malloc/cudaMalloc）
...
**验收条件**：
- [ ] 预热后 `malloc`/`free` 调用次数为 0（通过钩子验证）
...
**Must-Pass Gates（必须满足，否则 Phase 1 失败）**：

| 指标 | 目标 | 测试条件 | 验证命令 |
|------|------|----------|----------|
| 功能正确性 | 100% | 与 HF 参考实现对比 | `--gtest_filter=LlamaDecode.*` |
| 内存零分配 | 100% | Decode 循环 | 自定义 malloc 钩子统计 |
| 确定性 | 100% | 相同输入相同输出 | 100 次重复测试 hash 一致 |
| 无数据竞争 | 100% | TSAN 检测 | `cmake -DENABLE_TSAN=ON` |
| 内存无泄漏 | 100% | ASAN/LSAN 检测 | `cmake -DENABLE_ASAN=ON` |

**Stretch Targets（努力达成，允许部分妥协）**：
...
- 使用 `std::expected` 需确认编译器支持（GCC 12+/Clang 16+），否则使用自定义 `Expected<T, E>`
...
| 测试套件 | 覆盖范围 | 必须通过 |
|----------|----------|----------|
| `InferenceRuntimeContract.*` | API 边界与生命周期 | ✅ |
| `ModelConfigContract.*` | 配置解析与拒绝策略 | ✅ |
| `SafetensorsLoader.*` | 加载器与重排 | ✅ |
| `ReferenceCpuKernels.*` | 参考内核数值正确性 | ✅ |
| `QuantLinearInt8.*` | INT8 量化奇偶校验 | ✅ |
| `QuantLinearInt4.*` | INT4 量化奇偶校验 | ✅ |
| `KVCache.*` | 静态缓存管理 | ✅ |
| `LlamaDecode.*` | 端到端解码正确性 | ✅ |
| `CAbiRuntime.*` | C ABI 功能与内存安全 | ✅ |
| `InferenceRuntimeThreading.*` | TSAN 无竞争验证 | ✅ |
...
| 任务 | 交付物 | 验收标准 |
|------|--------|----------|
| 1.1 运行时契约 | `include/runtime/` 头文件布局 | 编译通过，无外部依赖 |
...
**文档维护者**: AetherMind 架构团队  
**审核状态**: ✅ Phase 1 可执行合同已就绪  
