...
TEST(LoadedModel, StoresConfigAndResolvedWeights) {
    ...
    auto model = std::make_unique<LoadedModel>(MakeLlamaConfig(1), std::move(index));
...
TEST(LoadedModel, PreservesResolvedWeightViews) {
    auto storage = std::make_shared<TestStorage>(80);
    for (size_t i = 0; i < storage->data.size(); ++i) {
...
TEST(LoadedModel, RetainsRawWeightBackingStorage) {
...

    {
        ...
        // Release the local shared_ptr. LoadedModel must hold its own copy.
        storage.reset();
        ASSERT_FALSE(weak_storage.expired())
                << "Backing storage was destroyed while LoadedModel still references it";

        // Views inside LoadedModel must still be valid and point to live memory.
        const auto& resolved_weights = model->GetResolvedWeights();
        EXPECT_TRUE(resolved_weights.embed_tokens.IsValid());
        EXPECT_EQ(resolved_weights.embed_tokens.data[0], std::byte{0});
        EXPECT_TRUE(resolved_weights.layers[0].attn.q_proj.IsValid());

        // Destroy the LoadedModel; this should release the last shared_ptr.
        model.reset();
        backing_destroyed = weak_storage.expired();
    }

    EXPECT_TRUE(backing_destroyed)
            << "Backing storage was not destroyed after LoadedModel went out of scope";
}

TEST(LoadedModel, StoresResolvedWeightsFromHfResolver) {
    ...
    auto model = std::make_unique<LoadedModel>(MakeLlamaConfig(1), std::move(*resolved));
