...
9. [ModelGraph 模块路径](#9-modelgraph-模块路径)
10. [关键技术点](#10-关键技术点)
11. [配置说明](#11-配置说明)
12. [测试方法](#12-测试方法)
13. [注意事项](#13-注意事项)
...
| Runtime tensor/state binding 接线 | 待补齐 | ❌ 未实现 |

**核心缺口**：`ModelGraph`、builder 和 lowering bridge 已落地，但模型加载完成后还没有自动串起 `BuildLlamaDense()`、`LowerModelGraph()` 和 `ExecutionPlanBuilder::Build()` 的生产级入口；`LoweredGraph` 保留的 graph value / state alias 也仍需完整接入运行时绑定。
...
ExecutionPlanBuilder::Build(runtime, model_instance, nodes)
     ...
     └─ for each node:
         ...
         │   ├─ MakeOperatorParamsForNode(node) → OpParams
         │   │   ├─ node.op_params (优先)
         ...
         │   │       ├─ std::get_if<Params>(&params) → 成功 → make_unique<OpClass>
         │   │       └─ 类型不匹配 → Status::InvalidArgument
         │   │
...
| 3.5a | `MakeOperatorParamsForNode` | `const ExecutionPlanNodeSpec&` | `OpParams` | 获取算子参数 |
| 3.5b | `OperatorRegistry::Create` | `OpType`, `const OpParams&` | `StatusOr<unique_ptr<Operator>>` | 从注册表创建算子 |
...
nodes.push_back(ExecutionPlanNodeSpec{
    ...
    .attrs = {},
    .op_params = OpParams{RmsNormOp::Params{.eps = 1e-6f}},
});
...
| # | 路径 | 当前状态 | 作用 |
|---|------|----------|------|
| P1 | 模型加载后自动触发 `BuildLlamaDense` / `LowerModelGraph` / `ExecutionPlanBuilder::Build` | 待集成 | 形成端到端生产编译入口 |
| P2 | 完整 runtime tensor/state binding 接线 | 待补齐 | 将 `LoweredGraph` 保留的 graph value 绑定到执行期资源 |
...
ModelInstance {+ backend_sidecar.packed_weights[]}
     │
     │  ⚠️ 缺口：缺少模型加载后自动编译到 ExecutionPlan 的生产入口
     │
     ▼ [手工构造 / LowerModelGraph]
...
ModelGraph
  ...
  ├─ nodes_: vector<GraphNode>
  └─ values_: vector<GraphValue>
       每个 GraphNode:
         ├─ op_type: OpType
         ├─ layer_index: uint32_t
         ├─ inputs/outputs: vector<GraphValueId>
         ├─ weight values: WeightValue payload
         ├─ attrs: ModelGraphAttrs (owned bytes)
         └─ op_params: OpParams
     │
     ▼  [已实现 bridge]
LowerModelGraph(graph, config)
     │
     ▼  [已实现]
vector<ExecutionPlanNodeSpec>
     │
...
| 里程碑 | 内容 | 状态 |
|--------|------|------|
| M1 | 定义 `GraphNode`/`ModelGraph`/`ModelGraphBuilder` + 测试 | ✅ 已实现 |
| M2 | 实现 `LowerModelGraph` bridge | ✅ 已实现 |
| M3 | 接入 `ModelInstance`，`ModelLoader` 完成后调用 Builder / Lowering | ❌ 未实现 |
...
src/model/graph/
  ...
  compilation/
    graph_lowering.cpp
    state_alias_plan.cpp
    graph_compiler.cpp
  optimization/
    graph_pass_manager.cpp
    graph_rewrite.cpp
    const_evaluator.cpp
    constant_folding_pass.cpp
    dead_code_elimination_pass.cpp
    silu_mul_fusion_pass.cpp

tests/unit/model/graph/
  test_model_graph_builder.cpp
  compilation/
    test_graph_compiler.cpp
    test_graph_lowering.cpp
  optimization/
    test_graph_pass_manager.cpp
    test_graph_rewrite.cpp
    test_constant_folding_pass.cpp
    test_dead_code_elimination_pass.cpp
    test_silu_mul_fusion_pass.cpp
```
