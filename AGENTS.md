...
| --- | --- | --- | --- |
| **graph** | `include/aethermind/graph/` | `src/graph/` | 通用 Graph IR、nodes/values/specs、GraphOpBuilder、优化 passes（GraphRewriteSession/ConstantFolding/DCE/SiluMulFusion/ConstEvaluator/GraphPassManager）、显式优化入口 `OptimizeModelGraph`、lowering（`LowerModelGraph` → `LoweredGraph`）、诊断 dump。**后端/设备/ISA 独立**：不得出现 Backend/Kernel/Workspace/DeviceType 执行层细节（DeviceType/IsaLevel/ExecPhase/WeightFormat/WorkspaceRequirement 等执行属性为 base 层纯数据契约，见 `base/device.h`、`base/kernel_attrs.h`、`base/workspace_types.h`）；StateAlias 仅以 must-alias 约束表达。graph/lowering 的 public 头引用 execution 模块纯数据契约头（`execution_node_spec.h`/`state_alias_plan.h`，仅依赖 base + operators + shape_inference，不含执行层细节），属 sanctioned bridge edge。 |
...
| **execution** | `include/aethermind/execution/` | `src/execution/` | 执行数据契约层：ExecutionPlan、ExecutionPlanNodeSpec（含 selector 执行属性、output_specs、runtime_checks）、StateAliasPlan、LayerRunner、ExecutionPlanBuilder。**Public headers 不得 include graph lowering**（LoweredGraph 仅前向声明）；bridge edges：`execution_plan_builder.cpp` 内部 include `graph/lowering/graph_lowering.h`（execution → graph，唯一编译期 include 边），`graph_lowering.h` 引用本模块纯数据契约头 `execution_node_spec.h`/`state_alias_plan.h`（graph → execution 契约依赖）。ExecutionPlanBuilder 的主要入口为 `Build(RuntimeContext, LoweredGraph)`，包含 ResolveStateAliases。 |
| **model** | `include/aethermind/model/` | `src/model/` | 模型加载与前端适配：HfModelConfig/HfDirectoryReader/HfModelValidator、ResolvedModelWeights、**LoadedModel**（只持有 config + resolved raw weights/backing storage）、ModelLoader（仅 HF I/O/validation/resolution）、**ModelGraphBuilder**（HF → semantic graph 唯一转换权威；`BuildLlamaDense` 路径显式拒绝 HF-only RoPE scaling types，none/linear 映射到 `RoPEScalingType`）、ModelCompiler（`LoadedModel → LoweredModelArtifact` 编排）。ModelLoader 不调用 graph、backend 或 prepack；`WeightPrepackPlanner` 将预打包权重写入 `PackedWeightStore`，是后续 graph-driven materialization/ExecutionPlan 接线前的兼容设施。 |
...
- execution → operators + shape_inference（public headers）；`execution_plan_builder.cpp` 内部 → graph/lowering（编译期 include 边）；graph/lowering → execution 纯数据契约头（`execution_node_spec.h`/`state_alias_plan.h`，bridge edge）
