...
int64_t ReadTokenId(const void* token_ids_data,
    ...
    if (dtype == DataType::UInt(32)) {
        return static_cast<int64_t>(
                static_cast<const uint32_t*>(token_ids_data)[index]);
    }
...
Status cpu::detail::EmbeddingKernel(const KernelContext& ctx) noexcept {
    const auto* args = static_cast<const cpu::detail::EmbeddingKernelArgs*>(
            ctx.kernel_params);
    ...
    for (int64_t token = 0; token < args->token_count; ++token) {
        ...
        const auto target_offset = static_cast<size_t>(token) *
                                   static_cast<size_t>(args->hidden_size);
...
Status BuildEmbeddingArgs(const KernelParamsBuildContext& context,
                          void* params_buffer) noexcept {
    const auto inputs = context.inputs;
    const auto outputs = context.outputs;
    if (inputs.size() != 2 || outputs.size() != 1) {
        return Status::InvalidArgument(
                "Embedding requires 2 inputs and 1 output");
    }
    ...
    if (token_count == 0) {
        ::new (params_buffer) cpu::detail::EmbeddingKernelArgs{
...
AM_REGISTER_KERNEL(EmbeddingFp32Scalar,
                   KernelDescriptor{
                           ...
                           .priority = 10,
                           .params_size = sizeof(cpu::detail::EmbeddingKernelArgs),
