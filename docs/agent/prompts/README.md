...
### 会话流转
- `docs/agent/prompts/handoff.md`：会话交接摘要模板。输出给下一位 agent 的最小必要上下文。
...
## 关系说明
- `new_session_template.md` 依赖 `handoff.md` 的输出结构，并要求按 `docs/agent/memory/README.md` 的路径规则加载 memory。
...
3. 根据任务定位主模块 `docs/agent/memory/modules/<module>/module.md`；若命中子模块，再加载 `docs/agent/memory/modules/<module>/submodules/<submodule>.md`。
4. 如果存在上一轮交接摘要，按 `docs/agent/prompts/handoff.md` 的结构读取并吸收当前状态。
5. 任务结束后，优先用 `docs/agent/prompts/memory_update_and_adr.md` 回写稳定结论；仅在首次建档或需要整体重写时使用 `docs/agent/prompts/generate_module_memory.md`。
