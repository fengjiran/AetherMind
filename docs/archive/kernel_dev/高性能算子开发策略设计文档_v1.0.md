...
- KernelDef / KernelSelector / KernelRegistry；
- Workspace 静态规划机制；
...
KernelSelector 负责根据 KernelDef 选择最合适 kernel。
...
- KernelDef 中应明确 `has_packed_weight`；
- kernel 不应直接读取模型结构对象；
...
- KernelDef 能否区分 packed/unpacked；
...
- `KernelDef`、`KernelCapability`、`KernelCandidate` 提供 `ToString()`；
...
```text
aethermind/kernels/kernel_def.h
aethermind/kernels/kernel_registry.h
aethermind/operators/ops/rmsnorm_op.h
```
