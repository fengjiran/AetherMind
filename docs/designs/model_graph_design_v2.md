...
struct GraphNode {
    ...
    // Milestone 1 compatibility shim only: std::any matches the current
    // OperatorRegistry params path. Milestone 2 must replace this with typed
    // OpParams or another schema-validated parameter object.
    std::any op_params{};
...
Milestone 1 可以短期保留 `std::any op_params`，仅作为兼容现有 operator registry 和各 operator `Params` 类型的迁移 shim。它不能成为 v2 IR 的长期参数表示。

`std::any` 过渡期必须遵守硬性限制：

- 只允许承载当前 registered operator 的语义 params；
- 不允许存放裸指针、backend handle、kernel selector、device config、lambda、opaque context 或 runtime resource；
- 不允许成为 serialization / debug dump contract 的一部分；
- 不允许被 semantic pass 当作无类型扩展字段随意读写；
- 所有新增 operator params 必须有明确的 typed `Params` 类型，并在 Milestone 2 进入 `OpParams` variant。

Milestone 2 必须将 `std::any` 收敛为 typed params：
...
ReplaceAllUses(old_value, new_value) 先解析 old_value 与 new_value；
...
- session 内临时 consumers cache，加速 `ReplaceAllUses`；
...
4. 将 weight 从 `GraphNode.weights` 迁移为 `WeightValue` payload，并保留现有 `ModelWeightBinding` 语义；
...
9. `std::any op_params` 仅作为 M1 兼容 shim 保留，并禁止放入指针、backend handle、kernel config、lambda、opaque context 或 runtime resource；
...
3. 将 `std::any op_params` 替换为 typed `OpParams` variant 或等价的 schema-validated typed parameter object；
...
GraphNode.weights: vector<ModelWeightBinding> -> WeightValue payload
...
1. 将 `std::any op_params` 替换为 typed `OpParams` variant 或等价的 schema-validated typed parameter object。
...
5. 不把裸指针、backend handle、kernel config、lambda、opaque context 或 runtime resource 塞进 `std::any op_params`；
