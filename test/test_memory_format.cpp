...
TEST(MemoryFormatTest, EnumValues) {
    EXPECT_EQ(static_cast<uint8_t>(MemoryFormat::Contiguous), 0);
    EXPECT_EQ(static_cast<uint8_t>(MemoryFormat::Preserve), 1);
    EXPECT_EQ(static_cast<uint8_t>(MemoryFormat::ChannelsLast), 2);
    EXPECT_EQ(static_cast<uint8_t>(MemoryFormat::ChannelsLast3d), 3);
...
TEST(MemoryFormatTest, GetChannelsLastStrides2D_4D) {
    ...
    std::vector<int64_t> actual_strides = get_channels_last_strides_2d(shape);
...
TEST(MemoryFormatTest, GetChannelsLastStrides3D_5D) {
    ...
    std::vector<int64_t> actual_strides = get_channels_last_strides_3d(shape);
...
TEST(MemoryFormatTest, IsChannelsLastStrides2D_Valid) {
    ...
    EXPECT_TRUE(is_channels_last_strides_2d(shape, strides));
}
...
TEST(MemoryFormatTest, IsChannelsLastStrides2D_Invalid) {
    ...
    EXPECT_FALSE(is_channels_last_strides_2d(shape, strides));

    // 测试不支持的维度大小
    std::vector<int64_t> shape_5d = {2, 3, 4, 5, 6};// 5维形状
    EXPECT_FALSE(is_channels_last_strides_2d(shape_5d, strides));
}
...
TEST(MemoryFormatTest, IsChannelsLastStrides3D_Valid) {
    ...
    EXPECT_TRUE(is_channels_last_strides_3d(shape, strides));
}
...
TEST(MemoryFormatTest, IsChannelsLastStrides3D_Invalid) {
    ...
    EXPECT_FALSE(is_channels_last_strides_3d(shape, strides));

    // 测试不支持的维度大小
    std::vector<int64_t> shape_4d = {2, 3, 4, 5};// 4维形状
    EXPECT_FALSE(is_channels_last_strides_3d(shape_4d, strides));
}
...
TEST(MemoryFormatTest, OperatorStream) {
    ...
    oss << MemoryFormat::Preserve;
    EXPECT_EQ(oss.str(), "Preserve");
    oss.str("");

    oss << MemoryFormat::Contiguous;
    EXPECT_EQ(oss.str(), "Contiguous");
    oss.str("");

    oss << MemoryFormat::ChannelsLast;
    EXPECT_EQ(oss.str(), "ChannelsLast");
    oss.str("");

    oss << MemoryFormat::ChannelsLast3d;
...
TEST(MemoryFormatTest, TemplateFunctionsWithDifferentTypes) {
    ...
    std::vector<size_t> strides_size_t = get_channels_last_strides_2d<size_t>(shape_size_t);
    EXPECT_EQ(strides_size_t.size(), 4);

    // 测试int类型
    std::vector<int> shape_int = {2, 3, 4, 5};
    std::vector<int> strides_int = get_channels_last_strides_2d<int>(shape_int);
