...
expected_behavior:
  ...
  - "默认启动路径为 `AGENTS.md -> docs/agent/memory/project.md -> docs/agent/handoff/workstreams/project__agent-memory-v1.1/<selected-active-handoff>.md`"
  - "`README.md` 属于按需升级读取，不在默认启动路径中"
  - "明确跳过任何 `module.md` 和 `submodule.md`"
  - "输出 Resume Gate，并在加载后询问用户确认"
validation_criteria:
  - "默认启动列表中不包含 `docs/agent/memory/README.md`"
  - "未读取 `docs/agent/memory/modules/**/module.md`"
  - "未读取 `docs/agent/prompts/handoff_template.md` 作为 handoff"
  - "引导阶段只执行根目录 `AGENTS.md` 第12节允许的只读引导操作，不执行非恢复/业务工具操作"
edge_cases:
  - "若无 active handoff：`resume_status` 为 `partial`，并说明\u201c从 memory 开始\u201d"
  - "若 `project.md` 缺失：`resume_status` 为 `blocked`（与 refined automation plan 一致）"
...
expected_behavior:
  ...
  - "默认启动路径为 `AGENTS.md -> docs/agent/memory/project.md -> docs/agent/handoff/workstreams/ammalloc__thread_cache/<selected-active-handoff>.md`"
  - "仅在 handoff 信息不足或需要模块不变量时，按需读取 `README.md`、`module.md` 和 `submodule.md`"
...
expected_behavior:
  ...
  - "按默认启动路径读取 `project.md` 和 `ammalloc__none` 下唯一选中的 active handoff 正文"
  - "仅在按需升级条件触发时读取 `module.md`，且不自动读取所有 `submodules/*.md`"
validation_criteria:
  - "没有对子模块目录执行全量扫描式加载"
  - "输出中明确“子模块：无”"
edge_cases:
...
regression_core: true
automation_potential: high
description: 所有必要文件和 active handoff 均存在时，默认启动最小路径，按需升级触发完整加载
preconditions:
  - "存在 `docs/agent/memory/modules/ammalloc/module.md`"
  - "存在 `docs/agent/memory/modules/ammalloc/submodules/thread_cache.md`"
...
expected_behavior:
  - "默认启动路径为 `AGENTS.md -> docs/agent/memory/project.md -> docs/agent/handoff/workstreams/ammalloc__thread_cache/<selected-active-handoff>.md`"
  - "`README.md`、`module.md`、`submodule.md` 属于按需升级读取，仅在以下情况之一触发：
       handoff 不足或 bootstrap_ready 为 false/missing、任务触及模块边界/所有权/线程安全/性能约束、需要处理 schema/兼容/写回规则"
  - "当 handoff 足以支撑低上下文恢复（bootstrap_ready: true）且无模块约束需要时，只加载默认启动路径"
  - "`resume_status` 为 `complete`"
validation_criteria:
  - "默认启动列表中不包含 `README.md`"
  - "只有在按需升级条件触发时才读取 module.md/submodule.md"
  - "任何顺序颠倒都判失败"
  - "handoff 选择使用 frontmatter 扫描 + 筛选 active + 排序 + 选中一个正文的算法"
edge_cases:
  - "若 handoff 有多个 active，先用元数据扫描发现所有候选，再按规则选择最新 active，其余仅作元数据不读正文"
  - "若 bootstrap_ready 为 false 或缺失，则触发按需升级读取"
...
regression_core: false
automation_potential: high
description: 模块工作不指定子模块时，默认加载 project 层和 `__none` handoff，模块记忆仅按需读取
preconditions:
  - "存在 `docs/agent/memory/modules/ammalloc/module.md`"
  - "存在 active handoff 于 `docs/agent/handoff/workstreams/ammalloc__none/`"
input: "继续 ammalloc"
expected_behavior:
  - "默认启动路径为 `AGENTS.md -> docs/agent/memory/project.md -> docs/agent/handoff/workstreams/ammalloc__none/<selected-active-handoff>.md`"
  - "仅在按需升级条件触发时读取 `module.md`"
  - "不读取任何 `submodules/*.md`"
validation_criteria:
...
regression_core: false
automation_potential: high
description: 按需升级需要目标子模块不变量、但对应 memory 缺失时，应按 partial 恢复
preconditions:
  - "存在 `docs/agent/memory/modules/ammalloc/module.md`"
  - "不存在 `docs/agent/memory/modules/ammalloc/submodules/new_bucket.md`"
  - "handoff 信息不足或任务明确需要 new_bucket 子模块不变量，已触发按需升级"
input: "继续 ammalloc new_bucket 的工作"
expected_behavior:
  - "先完成默认启动，再按需加载 `module.md`"
...
edge_cases:
  - "若 handoff 为 bootstrap_ready 且任务不需要子模块不变量，则未触发按需读取，子模块 memory 缺失不影响状态"
...
regression_core: true
automation_potential: high
description: 项目级工作默认从 project 层进入 handoff，显式跳过模块文档；可按需读取受影响的模块 memory 并在 Resume Gate 中记录
preconditions:
  - "存在 `docs/agent/memory/project.md`"
  - "存在 active handoff 于 `docs/agent/handoff/workstreams/project__docs-reorg/`"
input: "继续 project__docs-reorg 的工作"
expected_behavior:
  - "默认启动路径为 `AGENTS.md -> docs/agent/memory/project.md -> docs/agent/handoff/workstreams/project__docs-reorg/<selected-active-handoff>.md`"
  - "`README.md` 不在默认启动路径中"
  - "输出中明确列出 `module.md` 和 `submodule.md` 为跳过项"
validation_criteria:
  - "默认启动列表中不包含 `README.md` 或任何模块级 memory"
  - "任何模块级读取都判失败，除非触发按需升级条件并在 Resume Gate 中记录"
  - "不得把 `docs/agent/prompts/handoff_template.md` 当作 handoff 读取"
edge_cases:
  - "若 handoff 为空：状态应为 `partial`"
  - "若任务触及特定模块边界/所有权/线程安全/性能约束，允许按需读取受影响的模块 memory，但必须在 Resume Gate 的\u2018按需升级读取\u2019中记录"
...
### TEST-017: project.md 缺失时的阻塞处理
```yaml
id: TEST-017
category: 加载顺序
name: project.md 缺失时的阻塞处理
priority: P1
regression_core: false
automation_potential: medium
description: 根 memory 缺失时默认启动路径中断，应判为 blocked（与 refined automation plan 一致）
preconditions:
...
expected_behavior:
  ...
  - "`resume_status` 为 `blocked`"
  - "停止加载，输出阻塞原因"
  - "说明需要先创建或恢复 `project.md` 才能继续"
validation_criteria:
  - "不能错误判为 `partial`"
  - "Resume Gate 的缺失项必须明确写出 `docs/agent/memory/project.md`"
edge_cases:
  - "若 handoff 也缺失，则仍是 `blocked`"
failure_mode: "最可能是把任意上层 memory 缺失一律判成 partial"
```
...
input: "继续 ammalloc thread_cache 的工作"
expected_behavior:
  - "Resume Gate 至少包含：工作流类型、resolved_workstream_key、默认启动已加载文件、按需升级读取、跳过/缺失项、resume_status、handoff 路径"
validation_criteria:
  - "字段缺任一项即失败"
  - "字段名不可被同义词替换"
edge_cases:
...
regression_core: false
automation_potential: high
description: 默认启动所需文件及已触发的按需升级文件均存在时，应判为 complete
preconditions:
  - "project.md 和 active handoff 存在"
  - "若触发模块/子模块按需升级，对应 memory 文件也存在"
...
regression_core: false
automation_potential: high
description: 缺失 handoff，或已触发按需升级但所需子模块 memory 缺失时，应归入 partial
preconditions:
...
### TEST-025: 选择最新 active handoff（frontmatter 扫描 + 单一正文）
```yaml
id: TEST-025
category: Handoff管理
name: 选择最新 active handoff（frontmatter 扫描 + 单一正文）
priority: P0
regression_core: false
automation_potential: high
description: 同一 workstream 下同时存在 active、closed、superseded 时，允许扫描所有候选 frontmatter，但只读取最新 active 的正文
preconditions:
  - "同目录内存在 1 个 active、1 个 closed、1 个 superseded handoff"
  - "active 的 `created_at` 早于 closed，但状态合法"
input: "继续 ammalloc thread_cache 的工作"
expected_behavior:
  - "扫描所有候选 handoff 的 YAML frontmatter（仅元数据）"
  - "排除 closed 和 superseded"
  - "筛选 `status: active` 的有效条目"
  - "按 created_at 降序排序后读取唯一选中 handoff 的正文"
  - "用 active 内容生成当前接续目标"
validation_criteria:
  - "允许扫描全部候选的 frontmatter（元数据收集不计数为加载多个正文）"
  - "实际加载文件列表中只能出现选中的那一个 active handoff 正文"
...
### TEST-026: 多个 active handoff 异常处理（frontmatter 全扫 + 单一正文）
```yaml
id: TEST-026
category: Handoff管理
name: 多个 active handoff 异常处理（frontmatter 全扫 + 单一正文）
priority: P0
regression_core: true
automation_potential: high
description: 同一 workstream 存在多个 active 时，允许扫描所有候选 frontmatter，但只选择最新一个读取正文
preconditions:
  - "同一目录内存在两个 `status: active` handoff，且 `created_at` 不同"
input: "继续 ammalloc page_cache 的工作"
expected_behavior:
  - "扫描所有候选 handoff 的 YAML frontmatter（仅元数据）"
  - "筛选 `status: active` 后按 created_at 降序排序"
  - "仅读取最新 active handoff 的正文"
  - "输出告警，提示用户检查并收敛状态"
  - "建议将旧文件标记为 `superseded`"
validation_criteria:
  - "允许扫描全部候选的 frontmatter（元数据收集不计数为加载多个正文）"
  - "实际加载文件列表中只能出现选中的那一个 active 正文"
  - "不能静默选择"
  - "不能把多个 active 拼接合并"
edge_cases:
...
### TEST-027: frontmatter 损坏文件忽略（全量 frontmatter 扫描 + 排除无效候选）
```yaml
id: TEST-027
category: Handoff管理
name: frontmatter 损坏文件忽略（全量 frontmatter 扫描 + 排除无效候选）
priority: P1
regression_core: false
automation_potential: high
description: frontmatter 缺失或损坏的 handoff 文件不能参与恢复；允许扫描所有候选的 frontmatter，但只保留有效条目
preconditions:
  - "目录中存在一个损坏 frontmatter 文件和一个合法 active handoff"
input: "继续 ammalloc size_class 的工作"
expected_behavior:
  - "扫描所有候选 handoff 的 YAML frontmatter（仅元数据）"
  - "将损坏 frontmatter 标记为无效候选并排除"
  - "从有效条目中筛选 active 后排序，读取唯一选中 handoff 的正文"
  - "可选地提示存在损坏文件"
validation_criteria:
  - "损坏文件不应出现在实际加载文件列表"
  - "允许扫描损坏文件的 frontmatter（读取 frontmatter 失败即排除，不计为加载多个正文）"
  - "不能因损坏文件而整体失败"
edge_cases:
...
### TEST-031: 引导阶段允许只读操作，确认前禁止执行业务操作
```yaml
id: TEST-031
category: 用户确认机制
name: 引导阶段允许只读操作，确认前禁止执行业务操作
priority: P0
regression_core: true
automation_potential: high
description: 在 Resume Gate 前允许根目录 AGENTS.md 第12节定义的只读引导操作；非恢复/业务工具操作必须等待用户确认
preconditions:
  - "目标 workstream 可成功恢复到 `complete` 或 `partial`"
input: "继续 ammalloc thread_cache 的工作"
expected_behavior:
  - "引导阶段执行根目录 AGENTS.md 第12节允许的只读引导操作（解析类型、列出 handoff 路径、读取 AGENTS.md 和 project.md、扫描候选 frontmatter、读取唯一选中 handoff 正文、按需升级读取 memory 文档）"
  - "完成记忆加载与 Resume Gate 输出"
  - "给出推荐下一步"
  - "明确询问“记忆已加载，是否执行[推荐操作]？”"
  - "在用户未确认前不执行任何非恢复/业务工具操作"
validation_criteria:
  - "不得自动扫描代码、运行构建、运行测试或编辑文件"
  - "允许的读取仅限根定义的第12节所列的只读引导操作"
edge_cases:
  - "若 `resume_status` 为 `blocked`，也不得绕过确认直接进入新建模块流程"
failure_mode: "最可能是把“快速恢复”误实现成“快速恢复并立即开工”"
```

### TEST-032: 第一轮选择 workstream，第二轮确认后执行
```yaml
id: TEST-032
category: 用户确认机制
name: 第一轮选择 workstream，第二轮确认后执行
priority: P0
regression_core: false
automation_potential: high
description: 第一轮“继续”触发只读引导操作和 Resume Gate；只有第二轮明确确认词才授权非恢复/业务工具操作
preconditions:
  - "已完成一次合法 Resume Gate"
input:
  ...
  - "用户第二轮：继续"
expected_behavior:
  - "第一轮“继续”被识别为 workstream 选择/恢复请求，引导阶段执行根目录 AGENTS.md 第12节允许的只读引导操作"
  - "完成 Resume Gate 输出并询问确认后，第二轮 `继续` 被识别为显式确认"
  - "此后才允许执行非恢复/业务工具操作（如代码扫描、测试、编辑等）"
validation_criteria:
...
| TEST-031 | 引导阶段允许只读操作，确认前禁止执行业务操作 | 引导阶段允许只读操作，确认前禁止执行业务操作 |
...
- frontmatter 扫描 + 筛选 + 单一正文选择（TEST-025, TEST-026, TEST-027 覆盖）
