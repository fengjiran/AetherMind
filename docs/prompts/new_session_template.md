...
前置检查：
- 阅读 `AGENTS.md`。
- 阅读 `docs/prompts/handoff.md`，后续输出结构必须与其一致。
- 阅读 `docs/memory/project.md`；不存在时明确记为 `无项目级 memory`。
- 检查 `docs/memory/modules/[MODULE_NAME]/module.md` 是否存在；存在则加载，不存在时明确记为 `无主模块 memory`。
- 若 `[SUBMODULE_NAME|无]` 不为 `无`，先加载父模块 `docs/memory/modules/[MODULE_NAME]/module.md`，再检查 `docs/memory/modules/[MODULE_NAME]/submodules/[SUBMODULE_NAME].md`；不存在时明确记为 `无子模块 memory`。
- 如本轮形成稳定结论，按 `docs/prompts/memory_update_and_adr.md` 准备 memory 增量；若 `[是否有 ADR 增量: 是/否]` 为 `是`，同步准备 ADR 草案。
