...
## 概述
> **摘要**: 在现有 AetherMind 代码库中构建一个精简的 V1 桌面/服务器 CPU 本地推理运行时，用于 Llama 家族的仅解码器模型，采用 CPU 优先执行、HuggingFace Safetensors 导入、混合算子分发和稳定的 C++/C ABI。该计划刻意推迟服务化、多请求调度、GPU 后端和通用图运行时，直到单请求端到端 CPU 生成稳定为止。
> **交付物**:
> - 用于桌面/服务器 CPU 单进程、单模型、单请求解码的本地推理运行时模块
...
### 访谈摘要
- 产品形态: 面向桌面/服务器 CPU 的本地运行时优先，而非服务化系统
...
## 工作目标
### 核心目标
在 AetherMind 内部交付一个面向桌面/服务器 CPU 的本地运行时，能够加载一个受支持的 HuggingFace 风格的 Llama 家族检查点，将权重重排到内部 CPU 布局，使用 KV 缓存执行确定性单请求生成，并通过 C++ 和 C ABI 入口点暴露该流程。
