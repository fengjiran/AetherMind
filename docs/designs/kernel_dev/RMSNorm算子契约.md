...
版本：v0.1
状态：Phase 1 契约草案
适用范围：AetherMind Phase 1 CPU-first / Llama-family dense / FP32 RMSNorm
更新时间：2026-06-01
...
RMSNorm 对输入张量 `X` 的每一行独立归一化，并按 hidden 维度乘以权重 `gamma`。

$$
\operatorname{mean\_square}_s = \frac{1}{H}\sum_{h=0}^{H-1} X_{s,h}^2
$$

$$
\operatorname{scale}_s = \frac{1}{\sqrt{\operatorname{mean\_square}_s + \varepsilon}}
$$

$$
Y_{s,h} = X_{s,h} \cdot \operatorname{scale}_s \cdot \gamma_h
$$

Phase 1 不包含 bias / beta，不做 residual 融合，不做量化反量化，也不改变输入张量的 layout。
...
Phase 1 不支持空 tensor。`seq_len <= 0` 或 `hidden_size <= 0` 必须返回 `InvalidArgument`。
...
Phase 1 correctness 以 double reference 为基准：
...
- Phase 1 不做单行 hidden 维并行 reduction。
...
- 校验 `KernelInvocation::op_type == OpType::kRmsNorm`。
- 校验 CPU device。
