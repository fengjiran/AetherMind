...
按 [文档系统规范](documentation-guide.md) 执行：

- [ ] **文档变更 PR**：遵循 `docs/templates/` 模板；头部状态/日期/关联代码正确；`docs/README.md` 索引已同步；命名符合规范；运行 `python3 tools/verify_docs.py` 零告警
- [ ] **代码变更 PR**：按 [变更触发矩阵](documentation-guide.md#61-变更触发矩阵) 核对命中项（公共 API 变更 / 模块实现变更 / 新增删除模块 / 配置变更 / 并发契约变更 / 性能基线变化 / 规范变更 / 缺陷修复），命中项必须在本 PR 或显式标注的跟进 PR 中同步对应文档
- [ ] **过期文档**：发现文档与代码不符时立即修正文档（代码事实优先），不修改代码去迎合文档
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v2.1 | 2026-08-21 | 接入文档系统门禁：文档变更 PR 检查清单与代码变更触发矩阵核对（见 §2.11） |
