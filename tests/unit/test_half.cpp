...
TEST(HalfToFP32Test, HalfToFp32Bits_Zero) {
    // 测试正零和负零
    EXPECT_EQ(half_to_fp32_bits_ieee(0x0000), 0x00000000);// 正零
    EXPECT_EQ(half_to_fp32_bits_ieee(0x8000), 0x80000000);// 负零
}
...
TEST(HalfToFP32Test, HalfToFp32Bits_Normalized) {
    ...
    EXPECT_EQ(half_to_fp32_bits_ieee(0x48CD), 0x4119A000);//
}

TEST(HalfToFP32Test, HalfToFp32Bits_Infinity) {
    // 测试无穷大
    EXPECT_EQ(half_to_fp32_bits_ieee(0x7C00), 0x7F800000);// 正无穷
    EXPECT_EQ(half_to_fp32_bits_ieee(0xFC00), 0xFF800000);// 负无穷
}
...
TEST(HalfToFP32Test, Zero) {
    // 测试正零和负零
    EXPECT_EQ(half_to_fp32_value_ieee(0x0000), 0.0f); // 正零
    EXPECT_EQ(half_to_fp32_value_ieee(0x8000), -0.0f);// 负零

    // 验证符号位正确
    EXPECT_TRUE(std::signbit(half_to_fp32_value_ieee(0x8000))); // 负零应该有负号
    EXPECT_FALSE(std::signbit(half_to_fp32_value_ieee(0x0000)));// 正零应该没有负号
}
...
TEST(HalfToFP32Test, Normalized) {
    ...
    EXPECT_FLOAT_EQ(6.10351562e-5f, half_to_fp32_value_ieee(min_norm));

    // 测试1.0
    uint16_t one = 0x3C00;
    EXPECT_FLOAT_EQ(1.0f, half_to_fp32_value_ieee(one));

    // 测试-1.0
    uint16_t neg_one = 0xBC00;
    EXPECT_FLOAT_EQ(-1.0f, half_to_fp32_value_ieee(neg_one));

    // 测试最大规约数
    uint16_t max_norm = 0x7BFF;
    EXPECT_FLOAT_EQ(65504.0f, half_to_fp32_value_ieee(max_norm));

    // 1.0 in half: 0x3C00 -> 1.0 in float
    EXPECT_FLOAT_EQ(half_to_fp32_value_ieee(0x3C00), 1.0f);

    // -1.0 in half: 0xBC00 -> -1.0 in float
    EXPECT_FLOAT_EQ(half_to_fp32_value_ieee(0xBC00), -1.0f);
    ...
    EXPECT_NEAR(half_to_fp32_value_ieee(0x48CD), 9.6016f, 1e-3f); // 1849.0
}

TEST(HalfToFP32Test, Infinity) {
    // 测试正无穷
    uint16_t pos_inf = 0x7C00;
    EXPECT_TRUE(std::isinf(half_to_fp32_value_ieee(pos_inf)));
    EXPECT_GT(half_to_fp32_value_ieee(pos_inf), 0);

    // 测试负无穷
    uint16_t neg_inf = 0xFC00;
    EXPECT_TRUE(std::isinf(half_to_fp32_value_ieee(neg_inf)));
    EXPECT_LT(half_to_fp32_value_ieee(neg_inf), 0);
}

TEST(HalfToFP32Test, NaN) {
    // 测试NaN
    // 各种NaN值
    float nan1 = half_to_fp32_value_ieee(0x7C01);// 静默NaN
    float nan2 = half_to_fp32_value_ieee(0x7FFF);// 静默NaN
    float nan3 = half_to_fp32_value_ieee(0x7E00);// 信号NaN
    float nan4 = half_to_fp32_value_ieee(0xFC01);// 负静默NaN
...
TEST(HalfToFP32Test, EdgeCases) {
    ...
    // 验证规格化数和非规格化数的边界
    float last_denormal = half_to_fp32_value_ieee(0x03FF);// 最大非规格化数
    float first_normal = half_to_fp32_value_ieee(0x0400); // 最小规格化数
...
TEST(HalfToFP32Test, RandomValues) {
    ...
    EXPECT_FLOAT_EQ(0.333251953125f, std::bit_cast<float>(half_to_fp32_bits_ieee(random1)));

    uint16_t random2 = 0x4D12;// ~20.28125
    EXPECT_FLOAT_EQ(20.28125f, std::bit_cast<float>(half_to_fp32_bits_ieee(random2)));
}
...
TEST(HalfFromFP32Test, ZeroValues) {
    // 正零
    EXPECT_EQ(half_from_fp32_value_ieee(0.0f), 0x0000);

    // 负零
    EXPECT_EQ(half_from_fp32_value_ieee(-0.0f), 0x8000);
}
...
TEST(HalfFromFP32Test, NormalizedNumbers) {
    ...
    EXPECT_EQ(half_from_fp32_value_ieee(max_normal), 0x7BFF);
}
...
TEST(HalfFromFP32Test, Overflow) {
    ...
    EXPECT_EQ(half_from_fp32_value_ieee(overflow), 0x7C00);// 转换为无穷大

    // 超过fp16最小值的负数
    float underflow = -70000.0f;
    EXPECT_EQ(half_from_fp32_value_ieee(underflow), 0xFC00);// 转换为负无穷大
}

TEST(HalfFromFP32Test, Underflow) {
    // 下溢到零的正数
    float tiny_positive = 1e-10f;
    EXPECT_EQ(half_from_fp32_value_ieee(tiny_positive), 0x0000);

    // 下溢到零的负数
    float tiny_negative = -1e-10f;
    EXPECT_EQ(half_from_fp32_value_ieee(tiny_negative), 0x8000);
}

TEST(HalfFromFP32Test, Rounding) {
    // 测试舍入到最近偶数
    float value1 = 1.0009765625f;// 刚好在中间值，应该舍入到1.0
    EXPECT_EQ(half_from_fp32_value_ieee(value1), 0x3C01);

    float value2 = 1.001953125f;// 超过中间值，应该向上舍入
    EXPECT_EQ(half_from_fp32_value_ieee(value2), 0x3C02);
}

TEST(HalfFromFP32Test, SpecialValues) {
    // PI
    EXPECT_EQ(half_from_fp32_value_ieee(3.141592653589793f), 0x4248);

    // E
    EXPECT_EQ(half_from_fp32_value_ieee(2.718281828459045f), 0x4170);

    // 黄金比例
    EXPECT_EQ(half_from_fp32_value_ieee(1.618033988749895f), 0x3E79);
}
