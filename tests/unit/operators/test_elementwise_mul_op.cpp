...
TEST(ElementwiseMulOpPrepare, RejectsNullBackend) {
...
TEST(ElementwiseMulOpPrepare, ResolvesKernel) {
    ResetStubState();
    FakeBackend backend;
...
TEST(ElementwiseMulOpPrepare, PropagatesKernelResolutionFailure) {
    FakeBackend backend;
...
TEST(ElementwiseMulOpPrepare, RejectsResolvedNullKernelFunction) {
    FakeBackend backend;
...
TEST(ElementwiseMulOpRun, RejectsCallBeforePrepare) {
...
TEST(ElementwiseMulOpRun, RejectsWrongInputCount) {
    ResetStubState();
    FakeBackend backend;
    backend.resolve_result = MakeStubKernel();
...
TEST(ElementwiseMulOpRun, RejectsWrongOutputCount) {
    ResetStubState();
    FakeBackend backend;
    backend.resolve_result = MakeStubKernel();
...
TEST(ElementwiseMulOpRun, InvokesResolvedKernelAndForwardsAttributes) {
    ResetStubState();
    FakeBackend backend;
    backend.resolve_result = MakeStubKernel();
