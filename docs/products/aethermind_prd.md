...
- **模块所有权（源码目录-职责冻结）**：
  ...
  - **`model/`**：HF 加载/校验、`LoadedModel`、ModelLoader、**模型家族识别与图构建**（`ParseModelArchitecture` 是家族判定唯一权威；`BuildModelGraph` 是前端→语义图的唯一转换权威入口，按家族分发到 `BuildLlamaDense` 等 per-family builder，HF RoPE 字段在其中规范化为 typed `RoPEAlgorithmParams`，unknown type 拒绝）。ModelCompiler 归 compiler；ModelLoader 不执行 graph build、kernel resolve 或 prepack。权重重排/materialization 必须由优化图的具体 weight binding 驱动。`PackedWeightStore`/`PrepackWeightRequests` 仅为现有 ExecutionPlan packed-weight API 的兼容设施。
...
- **参考内核**：默认 Float32 累积，数值稳定实现作为正确性基准。CPU FP32 RoPE 使用 double 计算频率、角度、三角函数和旋转乘加，最终写回 Float32；与固定版本 HF 的 FP32 路径按误差门禁对照，不要求逐位一致，见 [RoPE 算子契约](../operators/rope/RoPE算子契约.md)。
